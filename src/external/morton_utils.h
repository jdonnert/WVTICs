...
#else
#include <x86intrin.h>
static inline uint32_t block2morton2D_32 ( uint32_t v )
{
    return _pdep_u32 ( v, 0x55555555u ) | _pdep_u32 ( v >> 16, 0xaaaaaaaau );
}
static inline uint32_t coord2morton2D_32 ( uint32_t x, uint32_t y )
{
    return _pdep_u32 ( x, 0x55555555u ) | _pdep_u32 ( y, 0xaaaaaaaau );
}
static inline uint32_t morton2block2D_32 ( uint32_t v )
{
    return _pext_u32 ( v, 0x55555555u ) | ( _pext_u32 ( v, 0xaaaaaaaau ) << 16 );
}
static inline void morton2coord2D_32 ( uint32_t v, uint32_t *x, uint32_t *y )
{
    *x = _pext_u32 ( v, 0x55555555u );
    *y = _pext_u32 ( v, 0xaaaaaaaau );
}
static inline uint64_t block2morton2D_64 ( uint64_t v )
{
    return _pdep_u64 ( v, 0x5555555555555555u )
           | _pdep_u64 ( v >> 32, 0xaaaaaaaaaaaaaaaau );
}
static inline uint64_t coord2morton2D_64 ( uint64_t x, uint64_t y )
{
    return _pdep_u64 ( x, 0x5555555555555555u ) | _pdep_u64 ( y, 0xaaaaaaaaaaaaaaaau );
}
static inline uint64_t morton2block2D_64 ( uint64_t v )
{
    return _pext_u64 ( v, 0x5555555555555555u )
           | ( _pext_u64 ( v, 0xaaaaaaaaaaaaaaaau ) << 32 );
}
static inline void morton2coord2D_64 ( uint64_t v, uint64_t *x, uint64_t *y )
{
    *x = _pext_u64 ( v, 0x5555555555555555u );
    *y = _pext_u64 ( v, 0xaaaaaaaaaaaaaaaau );
}

static inline uint32_t block2morton3D_32 ( uint32_t v )
{
    return _pdep_u32 ( v    , 0x09249249u )
           | _pdep_u32 ( v >> 10, 0x12492492u )
           | _pdep_u32 ( v >> 20, 0x24924924u );
}
static inline uint32_t coord2morton3D_32 ( uint32_t x, uint32_t y, uint32_t z )
{
    return _pdep_u32 ( x, 0x09249249u )
           | _pdep_u32 ( y, 0x12492492u )
           | _pdep_u32 ( z, 0x24924924u );
}
static inline uint32_t morton2block3D_32 ( uint32_t v )
{
    return _pext_u32 ( v, 0x9249249u )
           | ( _pext_u32 ( v, 0x12492492u ) << 10 )
           | ( _pext_u32 ( v, 0x24924924u ) << 20 );
}
static inline void morton2coord3D_32 ( uint32_t v,
...
static inline uint32_t coord2block2D_32 ( uint32_t x, uint32_t y )
{
    return ( x & 0xffff ) | ( y << 16 );
}
static inline void block2coord2D_32 ( uint32_t v, uint32_t *x, uint32_t *y )
{
    *x = v & 0xffff;
    *y = v >> 16;
}
static inline uint32_t coord2block3D_32 ( uint32_t x, uint32_t y, uint32_t z )
{
    return ( x & 0x3ff ) | ( ( y & 0x3ff ) << 10 ) | ( ( z & 0x3ff ) << 20 );
}
static inline void block2coord3D_32 ( uint32_t v,
                                      uint32_t *x, uint32_t *y, uint32_t *z )
{
    *x = v & 0x3ff;
    *y = ( v >> 10 ) & 0x3ff;
...
static inline uint64_t coord2block2D_64 ( uint64_t x, uint64_t y )
{
    return ( x & 0xffffffff ) | ( y << 32 );
}
static inline void block2coord2D_64 ( uint64_t v, uint64_t *x, uint64_t *y )
{
    *x = v & 0xffffffff;
    *y = v >> 32;
}
static inline uint64_t coord2block3D_64 ( uint64_t x, uint64_t y, uint64_t z )
{
    return ( x & 0x1fffff ) | ( ( y & 0x1fffff ) << 21 ) | ( ( z & 0x1fffff ) << 42 );
}
static inline void block2coord3D_64 ( uint64_t v,
                                      uint64_t *x, uint64_t *y, uint64_t *z )
{
    *x = v & 0x1fffff;
    *y = ( v >> 21 ) & 0x1fffff;
